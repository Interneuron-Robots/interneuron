...
public:
    ...
    {                        
        #ifdef INTERNEURON
        subscription_ = this->create_subscription<test_msgs::msg::TestString>(sub_topic, 1, std::bind(&IntermediateNode::callback, this, _1,_2));
        #else
        subscription_ = this->create_subscription<test_msgs::msg::TestString>(sub_topic, 1, std::bind(&IntermediateNode::callback, this, _1,_2));
        #endif
...
private:
    ...
    void callback(const test_msgs::msg::TestString::SharedPtr msg, const rclcpp::MessageInfo & msg_info) {
        RCLCPP_INFO(this->get_logger(), "msg_info source time: %ld, received time:%ld", msg_info.get_rmw_message_info().source_timestamp, msg_info.get_rmw_message_info().received_timestamp);
        std::string name = this->get_name();
        RCLCPP_INFO(this->get_logger(), ("callback: " + name).c_str());
        //gettimeofday(&ctime, NULL);            
        //trace_callbacks_->trace_write(name+"_in",std::to_string(ctime.tv_sec*1000+ctime.tv_usec/1000));            
