 ...
 * @LastEditTime: 2023-08-16 23:35:27
 * @LastEditors: Sauron
 */
...
namespace interneuron
{
	...
	struct TP_Info
	{
		...
		void add_log(std::string timepoint, uint64_t t0, uint64_t t1, uint64_t t2=0){
			//TODO error here
			this->logs_.insert_or_assign(timepoint, std::make_tuple(t0, t1, t2));// usually, t0 si the current time ,t1 is the reference time
			//logs_[timepoint] = std::make_tuple(t0, t1, t2);// usually, t0 si the current time ,t1 is the reference time
		}
		std::string output_log(){
			...
			for(auto it = this->logs_.begin(); it != this->logs_.end(); it++){
		...
		std::map<std::string,std::tuple<uint64_t,uint64_t,uint64_t>> logs_;//key is the id of TimePoint, value is the reference time of the point
		#endif
	};
